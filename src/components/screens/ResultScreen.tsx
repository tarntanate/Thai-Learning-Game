    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
      <GlassCard className="relative overflow-hidden text-center">
        ...
        <h1 className="thai-text-safe text-gradient mt-2 text-3xl font-bold">
          {result.cleared
            ? perfect
              ? "สุดยอดมาก! ตอบถูกทุกข้อ"
              : "เล่นจบแล้ว เก่งมาก!"
            : "หัวใจหมดแล้ว..."}
        </h1>
        <p className="mt-2 text-sm text-slate-500">
          {subjectMeta?.emoji} {subjectMeta?.shortLabel} ·{" "}
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Score
            label="ยอมแพ้"
            value={String(result.surrendered)}
            tone="text-indigo-500"
          />
          <Score
            label="หัวใจเหลือ"
            value={`${result.heartsLeft}/${MAX_HEARTS}`}
            tone="text-pink-500"
          />
        </div>
      </GlassCard>
      ...
      <GlassCard delay={0.08}>
        <h2 className="font-display mb-3 text-lg font-semibold text-slate-700">
          🎁 ของรางวัล
        </h2>
            ...
            {reward.levelsGained > 0 && (
              ...
              >
                ...
                <div className="mt-1 text-xs font-semibold text-rose-600">
                  🎁 โบนัสเลเวลอัป: ได้รับคุณครูช่วยเฉลย +{reward.levelsGained}{" "}
                  ชิ้น!
                </div>
              </motion.div>
            )}
...
function Score({
  ...
  return (
    <div className="rounded-2xl bg-white/65 px-3 py-3 shadow-inner">
      <div className={`font-display text-2xl font-bold tabular-nums ${tone}`}>
        {value}
      </div>
...
function Reward({
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl bg-white/65 px-4 py-3 shadow-inner">
      ...
      <div>
        <div className="font-display text-lg font-bold text-violet-500">
          {value}
        </div>
        <div className="text-[11px] text-slate-500">{label}</div>
      </div>
